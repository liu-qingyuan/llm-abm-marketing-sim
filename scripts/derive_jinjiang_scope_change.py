"""Derive a corrected Jinjiang Douyin comments scope offline.

No live API. No .env reads. This script reuses existing processed CSVs.
"""
...
def main() -> None:
    ...
    add_already_covered = add_vids & old_comment_covered_vids
    add_new_vids = add_vids - old_target_vids
    corrected_vids = base_keep_vids | add_vids
    covered_vids = corrected_vids & old_comment_covered_vids
    ...
    for vid in sorted(add_new_vids):
        ...
        new_manifest_rows.append({
            ...
            "scope_change_action": "added_top12_from_source_metadata",
        })
    ...
    audit = {
        ...
        "top12_already_covered_by_old_run": len(add_already_covered),
        "top12_new_unique_videos": len(add_new_vids),
        "top12_needs_comment_fetch": len(add_new_vids & needs_fetch_vids),
        ...
        "completion_status": "source_metadata_gap_for_top12" if source_metadata_gap else ("comment_backfill_needed" if needs_fetch_vids else "complete_from_existing_rows"),
    ...
    report.update({
        ...
        "stage_status": {
            **old_report.get("stage_status", {}),
            "comments": "derived_existing_rows_only" if partial else "derived_complete_from_existing_rows",
            "replies": "derived_existing_rows_only" if partial else "derived_complete_from_existing_rows",
            "profiles": "disabled",
        },
    })
    report["selection_metadata"] = {
        "collection_scope": "caption_hashtag_scope_change_remove_binguan_add_jian_derived",
        "selection_source": f"{SOURCE_RUN_ID}/videos.csv",
    ...
    top12_status_note = (
        ...
        if source_metadata_gap
        else f"`{ADD_TAG}` is present in the local source `videos.csv`; check `needs_comment_fetch_manifest.csv` "
        "before claiming comment/reply completeness."
    )
...
- skipped: `{SKIP_TAG}`
- added: `{ADD_TAG}` from local `videos.csv` only
- live API: not run
- profiles_collected: false
...
- 跳过：`{SKIP_TAG}`。
- 补充：`{ADD_TAG}`，仅来自本地 source `videos.csv`，不进行 live API 补采。
...
| top12 已在旧 comments run 覆盖 | {len(add_already_covered)} |
