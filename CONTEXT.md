...
Video Catalog 中除 Target Marketing Video 之外的视频。它们只用于构建历史评论网络、历史标签亲和度和用户样本来源；曝光抽签失败可以记录为背景内容占用推荐机会，但当前基础版本不对这些视频执行 runtime 排序或 LLM 决策。
...
平台在用户所属固定批次中决定是否展示 Target Marketing Video 的一次机会。`horizon=30` 表示 30 个固定推荐批次，不表示自然日；每个用户在整个 Final Research Report Run 中最多进行一次目标视频曝光抽签。
