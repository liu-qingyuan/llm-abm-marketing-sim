...
- additive v2 concrete Adapters 分别冻结 official DeepSeek `deepseek-v4-flash`（chat wire、thinking disabled）、Antigravity gateway 的两个 Gemini 条件、独立 Pi `kimi-coding/k3-256k` profile 与既有 Pi OpenAI `gpt-5.6-sol` profile；旧 OpenAI 四模型 allowlist 不扩写。五个条件对同一 Prompt variant 提交相同 client-submitted bytes，provider-specific wire 设置只进入 safe request evidence。
- v2 attempt evidence 分栏保留 Provider route、requested/observed identity、usage、retry timing 与 billing semantics。DeepSeek CNY ceiling 为 ¥25；gateway quota 与 Pi subscription nominal-cost reference 不与 CNY 汇总。typed error 不保存 raw Provider error、raw Prompt、credential 或 payload。
