...
Full-Pool source closure 不 materialize run-wide rows，而是按 batch 重放已提交 spool，独立复算 membership、candidate/pair/terminal 分母、coverage、feedback、observed-model 与 complete usage accounting；Formal schema 与 legacy Validation schema相互独立。Source manifest 同时关闭 operational journal/spool/attempt-ledger inventory。partial workspace 永不出现在 final destination；完整 staging 通过 row/hash/lineage closure 后才原子 rename。source 一旦关闭，重复 `run(...)` 只执行只读 closure并返回同一结果，不检查或调用 Adapter，也不改写 source 或 operational bytes。本 Module 仍不生成 Report candidate、release 或 canonical deployment。
