...
`ConcurrentRobustnessStudy.run(manifest, adapters_by_cell, output_dir)` 是 additive robustness Module 的唯一公开执行 Interface，并按 `concurrent-robustness-manifest-v1` 与 `concurrent-robustness-manifest-v2` exact dispatch；没有公开 per-cell planner、writer 或 resume Interface。
