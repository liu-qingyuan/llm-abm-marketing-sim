...
v2 manifest 始终关闭五个 canonical model conditions、P0–P3、20 cells、Formal 1,000 users / 30 batches / 三 message / 每 message Top20、每 cell 1,800 logical judgments、36,000 logical cap 与 108,000 physical-attempt cap。deterministic validation 可以沿用 fixture 的较小 horizon/capacity，但不能改变 20-cell topology，且同一 manifest 仍保留完整 Formal topology contract。`realization_source_identity` 只从冻结 sample identity、runtime-effective comment graph、message snapshot 与 `EngagementRealizationPolicy` 的 seed/rule canonical facts 派生；Prompt、Model、cell、batch、time step、执行顺序与模型清单不进入该 identity。

Formal Execution Preflight Module 暴露三个本地操作：`authorization_readiness(request)` 关闭全部非授权事实，`authorize_formal_execution(...)` 校验 exact canonical authorization 并写入新 execution plan，`validate_formal_execution_plan(...)` 在第一次 Provider 请求前重读 plan 及全部外部 hash-bound inputs。request 固定绑定 source artifact inventory、20 cell IDs、五个 requested/required-observed identities（只允许 `gemini-3.1-pro → gemini-pro-agent` 映射）、P0–P3 hashes、36,000/108,000 caps、单 worker/in-flight、timeout/backoff、四类独立 route caps、DeepSeek CNY ¥25 ceiling、credential route IDs 与 output identity。qualification/authorization 都要求 exact fields、canonical regular file、SHA-256、no-symlink 与最长 24 小时有效期；任何 deterministic fixture、GitHub label、Spec 或旧 canary 都不能替代这些输入。

缺少 authorization 时，Gate 在 source runtime closure、output workspace、Adapter map、credential 或 Provider 访问前抛出 typed `formal_authorization_required`，readiness 保持 `provider_calls=0`、`live_api_triggered=false`、`production_deploy_eligible=false`，并携带只允许替换三个占位值的 canonical authorization template 与 `ready-for-human` issue body。授权成功仍只表示该 execution identity 可运行；plan 明确保持 `release_authorized=false` 与 `deployment_authorized=false`。运行时 operational identity 绑定 plan/authorization hashes，完整 Formal execution manifest 内嵌自包含法律 lineage；Evidence 可在不重新读取已过期外部 authorization/qualification 文件的情况下验证当时闭合的 hashes、身份和scope。

v2 私有 pair ledger 依次持久化 `pending → reserved → attempting → judgment_persisted → realized_persisted → settled`。Provider Judgment 使用 cell-specific source identity；Realized terminal 使用 panel-wide source identity。concrete Provider Adapter 把 requested/observed identity、Prompt hash、wire/structured-output/reasoning 设置、usage 和安全错误事实归一化；同一模型的 P0–P3 共用私有 lane cooldown，单 pair 只允许三次 physical attempts。连接、timeout、408、409、429、5xx 与 malformed structured response 才能 retry，429/503 触发 lane cooldown；其他 4xx、认证、权限、identity drift 与 attempts exhausted 形成 `stopped`，unknown post-dispatch provenance 形成 `reconciliation_required`。Judgment 已落盘时恢复不再调用 Adapter，Realized terminal 已落盘时恢复只继续 kernel registration/settlement；两种失败都不能生成 runtime terminal 或提交当前 batch。kernel 继续拥有 full-batch barrier 和 next-batch feedback，只把 campaign-level 去重 realized-positive users 提交。
