...
离线基线与 `probability_v1` 在 artifacts 完成后调用内部 `FinalResearchReportWriter`。该 Module 使用独立的 `FinalResearchReportPayload` 和显式
...
`target_delivery_ranking_v2` 不复用这个概率报告合同，也不生成容易被误读的 v2 runtime HTML。它直接发布自洽的排名 runtime manifest；排序报告必须由显式支持 `below_delivery_capacity`、逐轮 ranking evidence 和独立 schema/version 的报告合同构建。

以下报告记录、Explainable payload v2 和 rebuild 约束只适用于离线基线与 `probability_v1` 历史 artifacts。
...
重建只把 `artifact_manifest.json`、`final_research_report_payload.json` 和 `runtime_summary.json`
作为权威输入。manifest 登记的其他 artifacts 只用于验证路径安全、文件存在和下载链接完整性；重建
不会读取用户 CSV/JSON 来重新推导决策，也不会运行仿真或调用 Decision Adapter。
...
发布前会交叉校验 schema、用户唯一性、样本/seed/summary/manifest 计数和全部下载目标。payload 与
