...
| Jinjiang v5 expand | `payload-v5` / `users-v5` | `final-research-ranking-runtime-v3` | `ranking-diagnostics-v2` | Prompt v3 | 本地 Validation only |

v5 同时持久化 `ranking-v5-expand-evidence-v1`。#76 完成后的 fresh writer 使用 `target_aggregate_engagement_reference.status=persisted`，但 `formal_research_evidence=false`；Decision execution evidence 仍为显式 `pending`，且 `production_deploy_eligible=false`。reader 继续只读兼容 #75 生成的双 `pending` v5 artifacts：该阶段必须没有 aggregate reference，并沿用冻结的 pending catalog 与报告语义。#77 负责填充 Decision evidence，#78 才能收口正式发布合同；当前 v1 production validator 继续明确拒绝 v5。
...
当前 Ranking v5 从 `seed_first_sample_audit.json`、逐轮 candidates/steps/outcomes、provider decisions、provider failures、ranking diagnostics 和 field trace artifacts 构建，并显式保存 sampling method/status、角色计数和 validation-expand evidence state。它不包含锦江 `interest_tags`；`historical_tags` 仍保持独立 Ranking evidence。payload 同时包含：
...
历史 v1/v2 重建继续使用 `artifact_manifest.json`、`final_research_report_payload.json` 和 `runtime_summary.json`。Ranking rebuild 联合校验 payload、users、runtime、diagnostics、Prompt 和 sample-audit schema token，以及精确 artifact set 后，才分流到冻结的 v3、v4 或 validation-only v5 reader；不存在 fallback。reader 随后交叉校验 sample audit、ranking steps/candidates/outcomes、ranking runtime summary、diagnostics summary、用户 CSV/JSON 和下载路径。v5 `persisted` 阶段要求 `top20_holdout_diagnostic.json` 中的 authoritative aggregate reference 与 diagnostics/payload 转发值完全一致且目标 `video_id` 匹配；兼容的 `pending` 阶段则要求三处都不存在该 reference。v3/v4/v5 各按自身冻结阶段的字段集合校验 catalog、trace、source records 与 manifest locator。所有路径都不会运行仿真或调用 Decision Adapter。
