...
| Jinjiang v5 | `payload-v5` / `users-v5` | `final-research-ranking-runtime-v3` | `ranking-diagnostics-v2` | Prompt v3 | 历史 Validation 或当前 v2 Formal 基线 |
| Jinjiang v6 | `payload-v6` / `users-v5` | `final-research-ranking-runtime-v4` | `ranking-diagnostics-v2` | Prompt v3 | Validation expand 或 v3 Formal candidate |
...
v6 是 additive evidence slice，精确 tuple 为 `final-research-ranking-report-payload-v6`、`final-research-ranking-users-v5`、`final-research-ranking-runtime-v4`、`ranking-diagnostics-v2`、`jinjiang-green-marketing-prompt-v3`、`ranking-v6-expand-evidence-v1 | ranking-v6-formal-evidence-v1` 和 `final-research-decision-execution-evidence-v2`。它复用 v5 用户 allowlist 与 artifact set，不迁移或改写历史 v3/v4/v5 artifacts。任意 crossed token 在模型解析与报告发布前失败关闭。
...
`validate_release(...)` 保持唯一 release-validation Interface，并按 `schema_version` 精确分流。历史 `abm-report-release-contract-v1` 仍可用于本地验证既有 v4 Validation evidence；`abm-report-release-contract-v2` 固定 `release_purpose=formal_research`，只接受完整 v5 Formal tuple，仍是当前 canonical release 的历史基线。`abm-report-release-contract-v3` 只接受完整 v6 Formal tuple：bare `['openai_compatible']`、`requested_model` 精确为 `gpt-5.4-mini`、独立 `observed_model` 精确为 `gpt-5.4-mini-2026-03-17`、每个 returned response 的 usage 完整、run-local invocation/response/Decision 计数闭合，并绑定 reason/context、holdout、manifest/download coverage 与全部 SHA-256。新增 `abm-report-release-contract-v4` 只接受冻结的 Concurrent Message Formal tuple：三条 authoritative message queues、Primary/Shadow prompt tokens、1,000-user / 3,000 eligible-pair / 1,800 exposure contract、100% pair terminal / paired Decision coverage、bare live `openai_compatible` provider path、完整 variant/total usage accounting，以及 concurrent report/download/manifest hash 闭合。v1/v2/v3/v4 都先执行只读 reader；校验过程不会重建或改写报告。v6 Validation candidate 即使 deterministic fixture 报告完整 model/usage，也不能通过 v3；Concurrent Message Validation candidate、crossed token、path escape、cache-only、rule-based、mock/live gate crossing 或 incomplete usage 也不能通过 v4。该唯一 mapping 来自 #87 的真实 qualification evidence；dated snapshot 是 requested alias 的精确返回身份，不描述为另一个 model family，也不用于推断价格或质量。未来 observed snapshot 的任何变化都必须由新 Ticket 重新 qualification，不能通过 wildcard、前缀、后缀或动态“最新”解析放宽。
...
`--contract` 必须是仓库内安全、非 symlink 的 regular file。deploy 先复制随机本地 snapshot，保留原始 source identity 检查，但对 snapshot 执行完整 v2、v3 或 v4 Formal validation；之后的 local hash 和 tar upload 只读取同一个只读 snapshot，避免 validation 与上传之间的 source mutation。任何 snapshot 校验失败都会在第一次 `ssh`、上传、容器或远程配置动作前拒绝 v1、Validation、rule-based、mock-provider、model/accounting mismatch、source mismatch、hash/path/symlink 或 schema crossing。通过本地 gate 后，既有 candidate health check、宿主检查、原子 `current` 切换、public acceptance 和失败恢复上一 release 的流程保持不变；public acceptance 额外按 contract 选择 Final Research 或 Concurrent Message 根标记、Formal status 和 artifact HEAD checks。
...
v6 同时 snapshot Provider response、successful Decision、observed model、usage 和 token counters。Responses/chat 返回后先构造只含 Decision text 与 allowlisted accounting facts 的 safe envelope，再在 Decision parsing 前计入 `provider_response_count`；因此 malformed Decision text 的 retry 仍保留前一次 response evidence。Transport failure 没有 envelope，只增加真实 SDK invocation。`provider_accounting` 是 `provider_metadata` 的 strict typed sibling；metadata allowlist 与 secret redaction 不放宽。每个 run 只持久化非负 delta，preflight、先前调用和 cache hit 不会被误算为本 run response evidence。

Complete usage 要求 strict non-boolean non-negative integers、`total_tokens == input_tokens + output_tokens`，且 reported cached input 不超过 input。Cached tokens 是 input 子集。Missing cached detail 保持 `null`；missing/malformed usage 不估算 token。Returned usage 不能覆盖无 response 的 transport failure，因此不是完整 billed cost、价格或节省成本证明。
...
新运行同时写出 `seed_first_sample_audit.json`。旧正式 run 保留 `network_augmented_sample_audit.json`，并在报告中标记为 Historical Network-Augmented Run；offline/mock 输出标记为 Validation Run。Validation Run 不表示已经执行 live provider 正式运行。v6 外部路径要求 `require_live_env=true`，且只在 registered bare/wrapped Provider leaf 经内部 SDK wrapper 产生 run-local external invocation delta 时写 `ranking-v6-formal-evidence-v1`；injected 或任意替代 builder client 保持 mock evidence。Accounting 不完整时仍如实保留 Formal status，但 `production_deploy_eligible=false`。Qualification evidence 由 #87 单独记录；真实 Formal Run 和 deployment 仍使用该 Ticket 的独立 human authorization。本地实现 Ticket 不构成授权。

历史 v5 manifest 使用 `final-research-ranking-runtime-v3`；v6 使用 `final-research-ranking-runtime-v4`，并在既有 `ranking_runtime_summary.json` 内加入 typed `provider_accounting` 与 reason/context aggregate，不新增独立 diagnostics artifact。冻结的 v3/v4/v5 reader 继续要求各自原 schema。Target Delivery runtime artifacts 不包含 `random_draw`、概率曝光语义、背景视频 impression 或 `background_content`。
...
- 历史 v5 `target_delivery_ranking_v2` runtime 使用 `final-research-ranking-report-payload-v5`、`final-research-ranking-users-v5` 和 `RankingUserReportRowV5`；新的 v6 writer 使用 payload v6、相同 users v5 与 runtime v4，在不扩大用户字段 allowlist 的前提下增加 Provider accounting 和 reason/context evidence。历史 v3/v4/v5 合同继续只用于既有 run rebuild。
...
历史 Ranking v5 从 `seed_first_sample_audit.json`、逐轮 candidates/steps/outcomes、provider decisions/actions/failures、ranking diagnostics 和 field trace artifacts 构建，并显式保存 sampling method/status、角色计数和 validation-expand evidence state。它不包含锦江 `interest_tags`；`historical_tags` 仍保持独立 Ranking evidence。payload 同时包含：
...
### Ranking payload v6 evidence

v6 在 v5 页面上增加五组并列 evidence：requested/observed model、invocation/response/Decision coverage、usage completeness 与 token aggregate、exact reason facts、Decision-visible PeerContext 和 selected Ranking context。Exact reason 对 CSV 解码后的 reason 使用原字符串相等，不 trim、不 case-fold、不 Unicode normalize、不 tokenize、不 translation；它不判断语义相似、理由质量、真实性或文案多样性。

Runner 在每次 Adapter 调用前记录实际 `PeerContext`，包括最终 `provider_failed` 的调用；v6 要求六项 counters 全为 0，context denominator 等于 `exposed_users`。Selected Ranking context 只从 `selected=true` candidate rows 聚合 `engaged_neighbor_count=0` 与 positive counts，denominator 同样等于 `exposed_users`。两组 evidence 并列展示，不能把 Ranking signal 描述为 Prompt input。

v6 reader 从 persisted Decision rows 重算 reason facts，从 persisted selected candidate rows 重算 Ranking context，并与 runtime/payload 中的 PeerContext aggregate 和 typed Provider accounting 交叉验证。`CachedDecisionAdapter` 命中可以形成 v6 Validation report，但 `runtime Decisions - Provider leaf successful Decisions` 会显示为 coverage gap；历史 cached `decision_source=provider` 不能证明本 run 返回了 response、model 或 usage。

Injected deterministic client、rule-based 或没有本次 external invocation 的 cache path 只生成 `ranking-v6-expand-evidence-v1`、`validation_run` 和 `production_deploy_eligible=false`。实际 live invocation 生成 `ranking-v6-formal-evidence-v1`；只有 bare Adapter、exact requested `gpt-5.4-mini`、exact observed `gpt-5.4-mini-2026-03-17`、完整 returned-response usage 与闭合 counts 同时成立时 eligibility 才为 true。模型身份与 usage 不证明价格、总 billed cost、研究质量或 deployment authorization。

历史 Ranking v3 只读取 Historical Network-Augmented 合同：`final-research-ranking-report-payload-v3`、`final-research-ranking-users-v3`、`final-research-ranking-runtime-v2`、`ranking-diagnostics-v1`、`jinjiang-green-marketing-prompt-v2` 与 `network-augmented-sample-audit-v1`。Seed-First v4 固定使用 v4 payload/users、同一历史 runtime/diagnostics/Prompt token 与 `seed-first-sample-audit-v1`，并要求包含历史字段的 catalog、trace 和 source-record artifacts。v5 只接受 payload/users v5、runtime v3、diagnostics v2、Prompt v3、Seed-First audit 和相同 artifact set 的完整 tuple；v6 只接受 payload v6、users v5、runtime v4、diagnostics v2、Prompt v3、匹配 sampling status 的 expand/formal evidence v1 和 Decision evidence v2。四个 reader 使用各自冻结的 payload、audit 和精确 artifact-set 合同；任意未知、缺失或交叉组合在模型解析前失败关闭。历史重建不会迁移 schema 或覆盖其他 artifacts。
