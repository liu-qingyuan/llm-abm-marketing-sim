...
`validate_release(...)` 保持唯一 release-validation Interface，并按 `schema_version` 精确分流。历史 `abm-report-release-contract-v1` 仍可用于本地验证既有 v4 Validation evidence；`abm-report-release-contract-v2` 固定 `release_purpose=formal_research`，只接受完整 v5 Formal tuple，仍是当前 canonical release 的历史基线。新增 `abm-report-release-contract-v3` 只接受完整 v6 Formal tuple：bare `["openai_compatible"]`、`requested_model` 精确为 `gpt-5.4-mini`、独立 `observed_model` 精确为 `gpt-5.4-mini-2026-03-17`、每个 returned response 的 usage 完整、run-local invocation/response/Decision 计数闭合，并绑定 reason/context、holdout、manifest/download coverage 与全部 SHA-256。v1/v2/v3 都先执行只读 reader；校验过程不会重建或改写报告。v6 Validation candidate 即使 deterministic fixture 报告完整 model/usage，也不能通过 v3。该唯一 mapping 来自 #87 的真实 qualification evidence；dated snapshot 是 requested alias 的精确返回身份，不描述为另一个 model family，也不用于推断价格或质量。未来 observed snapshot 的任何变化都必须由新 Ticket 重新 qualification，不能通过 wildcard、前缀、后缀或动态“最新”解析放宽。
