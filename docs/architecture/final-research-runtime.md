...
- `field_lineage_catalog.json`：全局字段定义，只保存一次 provenance、source fields、transformation、Declared Usage Stage 和限制；
- `user_field_trace.json`：紧凑的 per-user field trace index，保存 value status、locator、Actual Usage Stage、Prompt inclusion 和 omission reason；
- `field_source_records.json`：由 `user_id` 定位的 allowlisted 标签来源快照和必要 hashtag/text evidence，不保存 raw Prompt 或 Provider Payload。
...
- `interest_tags` 与 `historical_tags` 的 Field Lineage Catalog 和 per-user trace。两者使用独立 evidence 与 transformation，禁止互相回填；Prompt inclusion 来自实际 Prompt Field Summary 路径。
