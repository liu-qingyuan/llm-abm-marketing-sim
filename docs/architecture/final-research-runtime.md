...
v5 同时持久化 `ranking-v5-expand-evidence-v1`：Target aggregate reference 与 Decision execution evidence 在 #75 阶段均为显式 `pending`，`production_deploy_eligible=false`。后续 #76/#77 填充证据，#78 才能收口正式发布合同；当前 v1 production validator 明确拒绝 v5。
