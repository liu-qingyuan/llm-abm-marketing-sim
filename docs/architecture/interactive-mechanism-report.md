...
- v6 报告在 390px 与桌面 viewport 都必须无页面级横向溢出、无 incoherent overlap，并保持 model/usage/reason/context evidence 与 downloads 可读；仍不新增移动端专用插画或第二套交互模型。
...
- 不拆成独立机制站点和独立证据站点。
- 不实现独立的移动端信息架构、底部抽屉或移动端专用插画；移动 viewport 复用同一响应式报告合同。
- 不增加匿名、授权或多套用户详情视图。
- 不把预设权重描述为真实平台参数。
- 不把信号纳入描述为已观测结果影响。
