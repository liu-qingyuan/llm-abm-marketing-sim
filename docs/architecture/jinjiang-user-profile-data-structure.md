...
Implementation status: `UserProfile.latent_attributes`, processed variant generation, rule-based latent score, and aggregate latent group reporting are implemented
...
- Provider-backed LLM prompt 对 latent attributes 的 compact summary 接入；当前第一版 rule-based scoring 使用结构化 runtime contract。
...
graph TB
    ...
    subgraph llmView["后续 Provider-backed LLM 可见用户信息"]
...
Provider-backed LLM prompt 尚未在第一版接入 latent attributes。如后续接入，LLM 可见信息应和用户数据结构一致，分成两部分。
