...
| 模式 | 入口 | 读取场景 |
|---|---|---|
| 默认读取 | [current dataset：锦江 final dataset 审计](jinjiang-final-dataset-audit-20260624.md)、[current Formal release：Concurrent Robustness v6](jinjiang-concurrent-robustness-v6-local-release-20260813.md)、[current Retention：Retention final evidence](retention-cleanup-final-evidence-20260730.md) | 先确定数据 lineage、当前发布身份和保留/质量 closure。 |
...
六张 `mechanism-*.mmd` / `real-batch-mechanism.mmd` 由 package-internal Mechanism Presentation Module 确定性生成并保存在 `src/llm_abm_sim/report_assets/`；`concurrent-message-mechanism-semantic-masters-v4-review.md` 只是网页外整组审批预览。在 GitHub #185 合法整组批准以及后续 additive release 完成前，这些 bytes 不属于当前 canonical download set，也不改变 v1/v6 presentation。
