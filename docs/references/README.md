...
五张 `media-mechanism-*.png` 与 `editorial-mechanism-*-v1.webp` 继续冻结 Editorial v1；五组 versioned `editorial-mechanism-*-v2.png/.webp` 是本地 Editorial v2 默认 presentation 的 source/derivative assets。它们统一由 `src/llm_abm_sim/report_assets/` 拥有，source/derivative SHA-256、renderer compatibility goldens 和 published v1 report bytes 由代码与测试保护；v2 尚未因此获得部署授权。
