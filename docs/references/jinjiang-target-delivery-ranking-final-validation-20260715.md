...
在 #46-#48 完成样本与字段目录、排序与网络证据、聚合图表与用户代理指标解释后，#49 使用同一公开 `rebuild_final_research_report(run_dir) -> Path` Seam 重建正式 run。重建前后分别对 run 目录全部 25 个文件计算 SHA-256；文件集合保持 25 / 25，新增、缺失和 hash 变化均为 0。当前 `report.html` 已是目标解释版本，因此本次确定性重建连派生 HTML 都保持逐字节一致。
...
| `report.html` | `a3f374601b67daff6fcde5c9ebb1b2f6f86ec9d4f84aa688b389c8a944c89e6f` |
...
- mocked ranking Playwright：`3 passed`；正式 run Playwright：`1 passed`。
