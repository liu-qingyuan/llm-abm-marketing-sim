...
该命令支持历史 `abm-report-release-contract-v1` 的本地证据验证，以及 v2–v13 的版本化 Formal production验证。v2/v3/v4分别保护Final Research与Concurrent Message既有合同；v5/v6/v7保护Concurrent Robustness；v8/v9/v10分别只接受Full-Pool source-v1/source-v2/source-v3；v11只读兼容strict source-v4九行projection-v1；v12只接受同一source-v4产生的delivery-run projection-v2。v11/v12都会从persisted source、operator attempt chain、strict policy/settlement、30-batch spool、109,200 final successful Decisions、model/usage/physical accounting与同源result projection重新闭合facts；v12进一步要求270行，`Run = time_step + 1`并覆盖1–30，且roll-up回原九格与109,200 Exposure。v13只接受`formal_two_stage_realized` source、同源evidence/projection、Report production bytes、Historical artifacts和显式v12 baseline，并分栏闭合upstream live Provider accounting与realization zero-call accounting。它拒绝validation candidate、caller-injected facts、crossed identity、非canonical source/release bytes及missing/extra/symlink/hash drift。所有source directory都拒绝父级artifact path、symlink、FIFO/socket/device等non-regular entry、未声明文件、缺失download或hash不一致。v13已接入`--require-formal-production`与deployment facts；purpose、status与composite accounting仍由Release Module唯一解释，远端执行另由独立Deployment Authorization Interface控制。

在真实 strict run 前，可从最终 create-once manifest 生成零调用 handoff：

```bash
python scripts/compose_full_pool_v11_handoff.py \
  --repo-root . \
  --fresh-execution-manifest runs/<fresh-manifest>.json \
  --implementation-commit <clean-current-commit> \
  --release-id <planned-release-id> \
  --output runs/<independent-handoff>/execution-handoff.json
```
...
production deploy只能显式提供通过本地gate的v2–v13 Formal contract。v2–v12命令保持不变；v13还必须提供独立operational authorization：
...
scripts/deploy_abm_report.sh \
  ...
  --authorization runs/<independent-operational-authorization>.json
```

v13缺少`--authorization`时，命令在任何SSH、upload、remote write或public request前失败，并以`deployment authorization required: {...}`输出`abm-report-v13-deployment-readiness-v1` canonical JSON，供`ready-for-human` Ticket消费。authorization必须使用`abm-report-v13-deployment-authorization-v1` exact schema，声明`authorization_kind=explicit_operational_deployment`与`authorization_status=approved`，并绑定contract hash、release ID/identity、realized source identity、endpoint、host、remote root、port、container、image、`immutable-releases-atomic-current-v1` topology及旧release ID/path/report/manifest hashes。Spec、label、commit、readiness或普通确认不能替代该artifact；任一target变化都需要新授权。

部署脚本先复制随机本地snapshot，并让standalone validator在任何`ssh`、上传或远程配置前一次性产出validated deployment facts；report kind、release/domain identity、完整artifact hashes与public acceptance list只来自显式contract。v13再由Deployment Authorization Module关闭hash-bound plan；第一次SSH只读取fresh `current`，且必须在candidate write前与授权rollback identity完全相等。后续checksum、tar upload和public acceptance继续读取同一只读snapshot/facts，不扫描“最新”目录，也不按文件存在猜测版本。远端在atomic `current`切换前核对完整regular-file inventory、report/manifest/release identity、candidate container与Nginx；任一步失败都保持或恢复部署前fresh `current`，并重新核对旧report/manifest hashes。切换后，独立body verifier以固定八路分批下载并逐文件重验byte count与SHA-256；普通report kind继续完整验证所有不超过64 MiB的artifacts。Full-Pool使用固定`paged-v1`策略：report/manifest、全部CSV/Markdown/Mermaid及不超过4 MiB的元数据完整body/hash，其余bulk source与trace pages由exact public manifest、remote逐文件hash、public HEAD、candidate inventory与Playwright HEAD共同闭合。任一批内成员失败都会回到既有rollback。随后运行desktop/mobile、双语、键盘与交互Playwright acceptance；v13额外验证realized headline、two-stage inline SVG、fallback、`.mmd`与data downloads。成功输出只记录deployment UTC、fresh readback与public acceptance等operational evidence，不回写immutable release bytes。完整Interface见[Report Deployment Authorization and Rollback](../architecture/report-deployment.md)。
