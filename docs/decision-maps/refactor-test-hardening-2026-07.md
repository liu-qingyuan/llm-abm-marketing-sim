...
- `docs/04-开发验证/` 后续只保留薄 README 作为迁移索引。
...
Resolved. `docs/04-开发验证/01-06` 正文已迁移到职责型目录；旧 redirect 文件已删除，只保留 `docs/04-开发验证/README.md` 作为迁移索引：
...
`docs/index.md`、`docs/04-开发验证/README.md` 和当前阅读入口已更新到 canonical 路径；`04-开发验证/` 只保留迁移索引职责。
