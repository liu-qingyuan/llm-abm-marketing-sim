...
def _legal_history(origins: _Origins, records: tuple[dict[str, Any], ...], target: dict[str, Any] | None = None) -> CampaignProgress:
    ...
    for number, row in enumerate((*records, None), 1):
        ...
        payload = row["payload"]
        if row["kind"] == "final_model_parallel_accepted":
            from ._concurrent_recovery_final_model import validate_parallel_receipt
            validate_parallel_receipt(origins, state, payload, when, head)
