...
def _svg(trajectories: list[dict[str,Any]], message: str, threshold: int, *, count: bool = False) -> str:
    selected=[r for r in trajectories if r['message']==message and r['configuration'].endswith(f'-h{threshold}')]
    mean_key = 'mean_cumulative_count' if count else 'mean_cumulative_rate'
    low_key = 'seed_count_q025' if count else 'seed_q025'
    high_key = 'seed_count_q975' if count else 'seed_q975'
    minimum=min(0.,min(r[low_key] for r in selected))
    maximum=max(r[high_key] for r in selected)
    margin=max(1. if count else .02,(maximum-minimum)*.08)
    low,high=(0. if count else minimum-margin),maximum+margin
    unit = '累计互动人数' if count else '累计互动率 / 消息差值'
    def x(batch: int) -> float:
        return 65+(batch-1)/29*595
    def y(rate: float) -> float:
        return 275-(rate-low)/(high-low)*220
    pieces=[f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 710 370" role="img" aria-label="{message}, h={threshold}, {unit}">',
            '<rect width="710" height="370" fill="white"/>',
            f'<text x="65" y="25" font-family="sans-serif" font-size="16">{message} · h={threshold} · {unit}</text>']
    for i in range(5):
        value=low+(high-low)*i/4
        yy=y(value)
        label = f'{value:.0f}人' if count else f'{value*100:.1f}%'
        pieces.append(f'<path d="M65 {yy:.2f}H660" stroke="#ddd"/><text x="57" y="{yy+4:.2f}" text-anchor="end" font-size="12">{label}</text>')
    ...
    for w,color in enumerate(COLORS):
        ...
        if w==0:
            points=[f"{x(r['batch']):.2f},{y(r[low_key]):.2f}" for r in rows]
            points += [f"{x(r['batch']):.2f},{y(r[high_key]):.2f}" for r in reversed(rows)]
            pieces.append(f'<polygon points="{" ".join(points)}" fill="{color}" opacity="0.1"/>')
        points=' '.join(f"{x(r['batch']):.2f},{y(r[mean_key]):.2f}" for r in rows)
...
def render(root: Path, evidence: dict[str,Any], trajectories: list[dict[str,Any]]) -> None:
    ...
    diagnostics = evidence['path_diagnostics_summary']
    diagnostic_text = (
        f"与基准同seed配对的{diagnostics['paired_paths']}条路径中，"
        f"{diagnostics['same_final_exposure_set_as_baseline']}条最终曝光集合同一，"
        f"{diagnostics['changed_exposure_order_from_baseline']}条曝光顺序变化，"
        f"{diagnostics['changed_batch_assignments_from_baseline']}条跨批曝光分配变化（区别于仅同批内顺序变化），"
        f"{diagnostics['identical_endpoint_rates_to_baseline']}条终值互动率完全相同。"
        f"全部路径共{diagnostics['unique_final_exposure_sets']}种最终曝光集合；"
        f"每seed独立曝光顺序数量范围{diagnostics['unique_order_counts_per_seed_min']}–"
        f"{diagnostics['unique_order_counts_per_seed_max']}。"
        f"旧判断曝光数{diagnostics['historical_exposures']}，补采判断曝光数{diagnostics['topup_exposures']}。"
    )
    mechanical_text = ''
    if diagnostics['paired_paths'] == 2000 and diagnostics['same_final_exposure_set_as_baseline'] == 2000:
        mechanical_text = ('全部2000个配置配对最终曝光集合相同：固定判断库加共同随机数（CRN）使同seed、'
                           '同user-message的draw相同，因此终值严格一致是这组曝光集合下的机械结果；'
                           '这不代表过程或普遍不敏感，也不是推荐参数无效的证据。')
    precision_failed = sum(not r['mc_precision_met'] for r in comparisons)
    max_halfwidth_pp = 100 * max(
        (r[f'{kind}_ordinary_upper'] - r[f'{kind}_ordinary_lower']) / 2
        for r in comparisons for kind in ('difference', 'delta'))
    precision_text = (f'MC精度不足{precision_failed}/{len(comparisons)}条；最大普通95%均值区间半宽'
                      f'{max_halfwidth_pp:.4f}pp（目标0.5pp）。固定100 seeds，未据结果加种子。')
    trajectory_text = '；'.join(
        f"{r['message']}最大均值路径差：{r['configuration']}，第{r['batch']}批，"
        f"相对基准{r['delta_mean_cumulative_rate']*100:+.4f}pp"
        for r in evidence['trajectory_baseline_maxima'])
    trajectory_text += '。这些最大中间偏差为描述性结果，不是全时程显著性判据；终值相同不表示时间过程相同。'
    conclusions.extend([diagnostic_text, mechanical_text, precision_text, trajectory_text])
    conclusions = [text for text in conclusions if text]
    weight_rows = ''.join(
        '<tr><td>w'+str(i)+'</td>'+''.join('<td>'+value+'</td>' for value in label.split('/'))
        +'<td>1 / 3 / 6</td></tr>' for i,label in enumerate(WEIGHT_LABELS))
    charts=[]
    ...
    for message in messages[:3]:
        count_charts.append(f'<h3>{message} · 累计互动人数</h3><div class="charts">')
        for threshold in (1,3,6):
            name=f'{message}-count-h{threshold}.svg'
            (root/name).write_text(_svg(trajectories,message,threshold,count=True))
            count_charts.append(f'<img src="{name}" alt="{message}累计互动人数, h={threshold}"/>')
        count_charts.append('</div>')
...
<section><h2>参数矩阵</h2><p>每个权重点均与h=1/3/6交叉，共21配置；基准w0-h3。</p><table><tr><th>权重点</th><th>网络权重</th><th>反馈权重</th><th>内容权重</th><th>饱和阈值h</th></tr>{weight_rows}</table></section>
<section><h2>逐批动态曲线</h2>{''.join(charts)}</section>
<section><h2>累计互动人数曲线</h2><p>每消息single exposure，纵轴为该消息累计realized-positive人数；曲线为seed均值，非campaign跨消息去重人数。</p>{''.join(count_charts)}</section>
